...
- **組織概要**: 日次アクティブ / エンゲージユーザー数、機能採用トレンド
- **コード補完**: 言語別・エディタ別の提案数、承認数、承認率
- **チャット分析**: IDE Chat および GitHub.com Chat のメトリクス、チャットモード別の内訳
...
- Node.js 18 以上
- npm
- 適切な権限を持つ GitHub Personal Access Token（以下を参照）
...
| `GET /orgs/{org}/copilot/metrics/reports/users-1-day` | ユーザー別 1 日間利用レポート |
| `GET /orgs/{org}/copilot/metrics/reports/users-28-day/latest` | ユーザー別 28 日間利用レポート |
...
src/
├── app/
│   ├── api/              # API ルートハンドラ（GitHub API 呼び出しのプロキシ）
│   │   ├── members/
│   │   ├── metrics/
│   │   ├── seats/
│   │   └── usage-reports/
...
- **API Routes**（`src/app/api/`）は GitHub REST API へのリクエストをプロキシし、トークンをサーバー側に保持します。
- **Client Components**（チャート、テーブル）はプロパティとしてデータを受け取り、インタラクティブな可視化を行います。
